...
\begin{dfn}(Natural deduction)
	...
	\begin{enumerate}[label=\roman*)]
		...
		\item The system of natural deduction for $\lp$ has the following rules of inference. For any formulas $\varphi$, $\psi$ and $\zeta$ the following are valid inferences:
			...
			\end{enumerate}
		\item A deduction $\Gamma \vdash \beta$ is valid if there is a finite sequence of formulas such that the last element of the sequence is $\beta$ and if $\alpha_i$ is an element of the sequence before $\beta$ then one of the following holds:
			\begin{enumerate}[label=\alph*)]
				\item $\alpha_i$ is a tautology.

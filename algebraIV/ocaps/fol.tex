...
\subsection{Syntactic consequence: Natural deduction}
...
\begin{dfn}(Natural deduction)
	\hfill
	\begin{enumerate}[label=\roman*)]
		\item For a finite collection $\Gamma$ of $\flp$ and a formula $\beta$, an {\bfseries inference scheme} (or just inference) is the chain $\Gamma \vdash \beta$. If for the collection $\Gamma$ we write $\alpha_1, \ldots, \alpha_k$ then we write $\alpha_1, \ldots, \alpha_k \vdash \beta$.
		\iffalse
		...
		\item The system of natural deduction for $\lp$ has the following rules of inference. For any formulas $\varphi$, $\psi$ and $\zeta$ the following are valid inferences:
			\begin{enumerate}[label=\alph*), leftmargin=1.5cm]
				...
				\item[*:] Any tautology of the form $\varphi \cond \psi$ writtem as $\varphi \vdash \psi$.
			\end{enumerate}
		\item It can be stablished that $\Gamma \vdash \beta$ if there is a finite sequence of inferences such that the last element of the sequence is $\beta$ and if $\alpha_i$ is an element of the sequence before $\beta$ then one of the following holds:
			\begin{enumerate}[label=\alph*)]
				\item $\alpha_i$ is a valid inference.
				\item $\alpha_i$ is an element of $\Gamma$.
				\item $\alpha_k = \alpha_j \vdash \alpha_i$ for some $k,j < i$.
			\end{enumerate}
	\end{enumerate}
\end{dfn}
...
\begin{ejem}
	Prove $\alpha, \alpha \cond \beta, \beta \cond \gamma \vdash \gamma$.
	\begin{proof}
		We construct the sequence:
		\begin{enumerate}[label=\arabic*. ]
			\item $\alpha$
			\item $\alpha \cond \beta$
			\item $\beta$
			\item $\beta \cond \gamma$
			\item $\gamma$
		\end{enumerate}
	\end{proof}
\end{ejem}
...
\begin{thm}[Deduction theorem]
	If $\Gamma, \varphi \vdash \psi$ then $\Gamma \vdash \varphi \cond \psi$
\end{thm}
...
\begin{ejem}
	...
	\begin{proof}
		...
		\begin{enumerate}[label=\arabic*. ]
			\item $\alpha, \neg\neg\neg\alpha \vdash \neg\alpha$
			\item $\alpha \vdash \neg\neg\neg\alpha \cond \neg\alpha$
			\item $\alpha \vdash \alpha \cond \neg\neg\alpha$
			\item $\alpha \vdash \neg\neg\alpha$
		\end{enumerate}
	\end{proof}
\end{ejem}

We introduce the following notation used in \cite{logicadeaño}. When making a assumptions (adding a formula to our collection), a vertical line extends for the subsequent derivations and then a horizontal line closes with the conclusion. So we write the inference rules like:
...
\begin{tikzpicture}
	...
	\draw[thick] ([yshift=0pt]reneg-3-1.north west) -- ([yshift=0pt, xshift=60pt]reneg-3-1.north west);
	...
	\draw[thick] ([yshift=7pt]ricond-3-1.north west) -- ([xshift=0pt]ricond-4-1.south west);
	\draw[thick] ([yshift=-4pt]ricond-2-1.north west) -- ([yshift=-4pt, xshift=20pt]ricond-2-1.north west);
	\draw[thick] ([yshift=0pt]ricond-5-1.north west) -- ([yshift=0pt, xshift=60pt]ricond-5-1.north west);
	...
	\draw[thick] ([yshift=0pt]recond-4-1.north west) -- ([yshift=0pt, xshift=60pt]recond-4-1.north west);
\end{tikzpicture}
\end{center}
...
Of course, the full definition and formal notion of derivation, that is, syntactical consequence is explored in more depth in the appendix. When we stablish synctatic consequence we also stablish semantic consequence, that is, a tautology. These notions are equivalent for the natural deduction system as well as tableaux and sequent calculus, other inference systems.
So, when we stablish $\Gamma \vdash \beta$ we also stablish $\Gamma \vDash \beta$ and viceversa.

\section{First order predicate logic}
\subsection{Formal language and models}
\subsection{Free and bound variables}
\subsection{Natural deduction for FOL}

\section{Theories and axiomatic systems}
\subsection{First order arithmetic}
\subsection{First order NBG set theory}
\subsection{First order mathematical structures}

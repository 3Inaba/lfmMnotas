...
Se presenta la teoría de conjuntos Zermelo-Fraenkel (ZF) con el Axioma de Elección (ZFE) de manera axiomática a través de un lenguaje formal. Decimos que el lenguaje formal de la teoría de conjuntos (o simplemente el lenguaje TC) consiste de los siguiente símbolos \cite{breton}:
\begin{enumerate}[label=\arabic*.]
...
Una fórmula del lenguaje TC es una cadena finita de símbolos (finita en su sentido común) de forma que \com{tenga sentido}. Dicho de otro modo, haciendo las fórmulas atómicas como aquellas formadas con los relatores y las variables, digamos por ejemplo $x = y$ o $a \in b$, una cadena de símbolos será una fórmula si se puede construir a partir de otras fórmulas bajo ciertas reglas. Dichas reglas son, por ejemplo, que si $\varphi$ es una fórmula entonces $\forall x (\varphi)$ es una fórmula. Por ejemplo \[\forall x \exists z (x \in y \cond \neg x = z)\] se construye a partir de $x \in y$ y $x \not = z$. De la lógica clásica, diremos que $\varphi \cond \psi$ y $\neg \varphi$ es una fórmula si $\varphi$ y $\psi$ son fórmulas, por lo que $x \in y \cond \neg x = z$ es una fórmula. Luego $\exists z ( x\in y \cond \neg x = z )$ es una fórmula y entonces $\forall x \exists z (x \in y \cond \neg x = z)$ es una fórmula.

Otras cadenas como $\forall x \land \in z \neg$ no son fórmulas.

Abreviamos $\neg x = y$ por $x \not= y$, $\neg x \in y$ por $x \not\in y$, $\forall z (z \in x \cond z \in y)$ por $x \subseteq y$ y $\neg x \subseteq y$ por $x \not\subseteq y$. De manera similares, podemos introducir nuevos símbolos para representar fórmulas de nuestro lenguaje formal.
...
Por ejemplo, la fórmula $\exists y (x \not= y)$ tiene variable libre $x$ y variable ligada $y$. Intuitivamente, una fórmula con variable libre $x$ se interpreta como una \com{propiedad}. Esto nos permite hablar de propiedadades mientras podamos llevarlas al lenguaje TC. Si le es posible definir lo que significa \com{ser un perro rabioso} en el lenguaje TC entonces le es posible construir la colección de los perros rabiosos como sigue 
...
Vale la pena interpretar la definición anterior. Un término clase o simplemente una \textbf{clase} es un objeto que no es parte del lenguaje TC pero es de utilidad para hablar sobre la teoría que se construye a traves de él. Las clases se cuantifican sobre conjuntos, informalmente, son colecciones de conjuntos. Aquí $\mid$ se interpreta como \com{tal que} de modo que la definición nos dice que la clase $A$ es la colección de los conjuntos $x$ tales que $x$ es libre en $\varphi$, o bien, tales que $\varphi(x)$ (que se interpreta como \com{$x$ cumple la propiedad $\varphi$}).

Podemos describir la \textbf{clase de todos los conjuntos} por $\uclass := \{x \mid x=x\}$ y entonces convenir en que $x \in \uclass$ significa que $x$ es un conjunto, y en general, decir que si $x \in A$ entonces $\varphi(x)$.

Para clases $A$ y $B$ se escribe $A \cap B$ por $\{x \mid x \in A \land x \in B\}$ y $A \subseteq B$ por $\forall x (x \in A \cond x \in B)$.

A continuación, utilizando el lenguaje que acabamos de definir (de manera poco formal), se introducen algunos axiomas de la teoría ZFE:
...
Finalmente, cuando se dice que \com{formalizamos} un concepto lo que queremos decir es que definimos el concepto en un lenguaje formal. En nuestro caso, formalizar algo como los sistemas de ecuaciones será abstraer lo que son y definirlos en el lenguaje TC. Claramente esta no es la única formalización posible, conociéndose otros lenguajes formales o definiendo uno.
